...
Usage:
  python npy_player.py                        # play output/keypoints/train/
  python npy_player.py output/keypoints/val   # play a specific directory
  python npy_player.py path/to/file.npy       # play a single file
  python npy_player.py --split test           # shortcut for a split
  python npy_player.py --fps 25               # set playback frame rate
...
def play_file(npy_path: Path, fps: float, label_map: dict) -> str:
...
def main():
    ...
    parser.add_argument("target", nargs="?",
                        help=".npy file or directory (default: output/keypoints/train)")
    ...
    if args.target:
        ...
        target = BASE_DIR / "output" / "keypoints" / args.split
    else:
        target = BASE_DIR / "output" / "keypoints" / "train"
    ...
    while 0 <= idx < len(files):
        ...
        action = play_file(f, fps=args.fps, label_map=label_map)
        if action == "quit":
            break
